...
class OpenApiDocumentationTest {
    ...
    void shouldExposeDocumentedApiContract() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
            ...
            .andExpect(jsonPath("$.paths", hasKey("/api/parts/count")))
            .andExpect(jsonPath("$.paths", hasKey("/api/parts/search")))
            .andExpect(jsonPath("$.paths", hasKey("/api/equipaments")))
            .andExpect(jsonPath("$.paths", hasKey("/api/equipaments/{id}")))
            .andExpect(jsonPath("$.paths", hasKey("/api/equipaments/count")))
            .andExpect(jsonPath("$.paths", hasKey("/api/equipaments/search")))
